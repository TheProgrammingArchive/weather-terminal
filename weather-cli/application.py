...
class Application():
    ...
    def start_application(self):
        ...
        keywords = ['SETTINGS', 'CLEAR()','WEATHER TODAY', 'WEATHER NOW', 'WEATHER TENDAY', 'DETAILED TENDAY', 'TIME -N', 'DATE -T', 'WEATHER -T', 'WEATHER -DSC', 'WEATHER -PRS', 'WEATHER -WS', 'WEATHER -HDU', 'WEATHER -DEW', 'WEATHER -PAR','WEATHER -UV', 'WEATHER -VIS', 'WEATHER -MNP', 'LOC -P', 'LOC -T', 'LOC -FAV']
        ...
        while True:
            ...
            if command_arg.upper() in keywords:
                ...
                weather_tendayfdtl = weatherTenDay(prefix_tenday)
                ...
                if command_arg.upper() == 'SETTINGS':
                    ...
                    print(f'Weather description: {trpt}')
                    ...
                    if confirmation.upper() == 'Y':
                        with open('gotloc.txt', 'w') as ftrnc:
                            ftrnc.seek(0)
                            ftrnc.truncate()
                            ...
                            ftrnc.write(f'{location_Get}: {x}')

                            print('\nDetails changed successfully!\n')
                            print('Please relaunch the application to save changes!')
                            ftrnc.close()
                            exit()
                    else:
                        continue
